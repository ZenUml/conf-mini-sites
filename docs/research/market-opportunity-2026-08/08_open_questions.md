...
- **T01 implementation audit (2026-08-04).** Not implemented: local `feat/t01-single-root-index-bundle` still equals `origin/master` at `2bf138d`, the remote branch is absent, no PR exists, and current unit/API/UI expectations still reject one file as `BUNDLE_NOT_MULTIFILE`. No T01 acceptance run, shipment, or online behavior verification exists; wave 1 remains locked.
...
|---|---|---|---|---|---|
| T01 | P0 | Accept a single root `index.html` bundle | Recent AI/report evidence is frequently one-file; current validator rejects it (E088, E097, E104, E120) | Unit and E2E tests accept exactly one `index.html` while preserving folder validation, limits, secret scan, and CSP | **Approved 2026-08-03; not implemented as of 2026-08-04.** Isolated local branch equals `origin/master` (`2bf138d`), with no remote branch, diff, or PR; current source/tests still reject one file. No acceptance run, shipment, or online verification. Engineering contract: [`docs/specs/2026-08-03-t01-single-root-index-bundle-handoff.md`](../../specs/2026-08-03-t01-single-root-index-bundle-handoff.md) |
