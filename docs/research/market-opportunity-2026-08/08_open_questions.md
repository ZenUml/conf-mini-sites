...
| Q4 | Exact production CSP / sandbox / same-origin asset fetch behavior | Ceiling on P scores; marketing limits copy | Code: E029 (`connect-src 'self'`). Local multi-file demo `samples/static-qa-report/` proves relative CSS/JS/JSON/img (`paths OK` badge). Still need lite-dev publish when Forge credentials available | **Mostly closed** locally; lite-dev serve spot-check still open |
