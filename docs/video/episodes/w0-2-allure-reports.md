...
Status: brief ready for script drafting; fixture production/acceptance is pending mini-sites-dev confirmation
...
The final fixture package must include a short README with generation commands, tool versions, test source, expected pass/fail counts, offline verification command, known exclusions, and SHA-256. It must be usable for the video, the S06 guide download, and discovery outreach without modification.
...
- mini-sites-dev owns fixture generation and compatibility confirmation. The producer must not invent commands or repair generated source to make it pass.
